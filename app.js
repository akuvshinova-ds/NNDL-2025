...
async function predict() {
    ...
    try {
        ...
        // Ensure predValues are numbers and handle array format
        const processedPredictions = predValues.map(pred => {
            // Handle both scalar and array outputs
            const probability = Array.isArray(pred) ? pred[0] : pred;
            return typeof probability === 'number' ? probability : parseFloat(probability) || 0;
        });

        // Create prediction results
        const results = preprocessedTestData.passengerIds.map((id, i) => ({
            PassengerId: id,
            Survived: processedPredictions[i] >= 0.5 ? 1 : 0,
            Probability: processedPredictions[i]
        }));
...
async function exportResults() {
    ...
    try {
        // Get predictions - ensure we extract numbers from tensor
        const predValues = await testPredictions.array();
        ...
        preprocessedTestData.passengerIds.forEach((id, i) => {
            submissionCSV += `${id},${processedPredictions[i] >= 0.5 ? 1 : 0}\n`;
        });

        // Create probabilities CSV (PassengerId, Probability)
        let probabilitiesCSV = 'PassengerId,Probability\n';
        preprocessedTestData.passengerIds.forEach((id, i) => {
            probabilitiesCSV += `${id},${processedPredictions[i].toFixed(6)}\n`;
        });
